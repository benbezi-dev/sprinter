...
(function (root) {
  ...
  const LEVELS = [
    ...
    { name: 'Jeux olympiques', theme: 'day', pool: 'sprint',
  ...
  // Les etapes locales reunissent un plateau varie ; le championnat du monde,
  // les Jeux et la finale ZEZE alignent des sprinteurs noirs, comme les vraies
  // finales du 100 m.
  const SKIN_POOL = {
    divers: ['ebene', 'cacao', 'acajou', 'noisette', 'bronze', 'ambre',
             'miel', 'olive', 'sable', 'clair', 'porcelaine'],
    sprint: ['ebene', 'cacao', 'acajou', 'noisette', 'bronze']
  };
  const HAIR_COLS = {
  ...
  const ZEZE = {
    ...
    'Ryan ZEZE': look({ build: 'm', skin: 'noisette', jersey: [48, 132, 232],
      shorts: [24, 30, 52], shoe: [250, 224, 70], hair: 'crop', h: 1.78 }),
    'Mickeal ZEZE': look({ build: 'm', skin: 'cacao', jersey: [44, 190, 128],
      shorts: [22, 34, 32], shoe: [246, 126, 46], hair: 'flattop', h: 1.85 }),
    'Herman ZEZE': look({ build: 'm', skin: 'acajou', jersey: [246, 150, 40],
      shorts: [34, 26, 22], shoe: [126, 226, 250], hair: 'shaved', h: 2.00 }),
    'Greta ZEZE': look({ build: 'f', skin: 'bronze', jersey: [162, 92, 232],
    ...
    'Victoire ZEZE': look({ build: 'f', skin: 'noisette', jersey: [236, 96, 178],
      shorts: [36, 22, 36], shoe: [246, 126, 46], hair: 'ponytail', h: 1.63 })
  };

  const PLAYER_LOOK = look({ build: 'm', skin: 'cacao', jersey: [248, 205, 74],
  ...
  Runner.prototype.press = function (key) {
    if (this.finished || this.stumbleTimer > 0) return false;
    let stumbled = false;
    if (this.lastKey === key) {
  ...
  Runner.prototype.stepPlayer = function (dt, elapsed) {
    ...
    if (this.finished) {
      ...
      this.d += this.v * dt;
      this.stride += this.v * dt * (TAU / this.strideLength());
      return;
    }
  ...
  function pose(r) {
    ...
    function leg(q) {
      const th = gait(GAIT.thigh, q) * A;
      const kn = gait(GAIT.knee, q) * (0.42 + 0.58 * A);
      const an = gait(GAIT.ankle, q) * (0.50 + 0.50 * A);
      return [th, th + kn, th + kn + an];
    }
    function arm(q) {
      const ua = gait(GAIT.arm, q) * (0.55 + 0.45 * A);
    ...
    const bob = -0.026 * A * Math.cos(2 * (p - 0.75));
    const yawHip = -0.13 * A * Math.sin(p);
    const yawTop = 0.17 * A * Math.sin(p);
    const sway = 0.012 * A * Math.sin(p);

    const shY = fem ? 0.130 : 0.154;
    const hipY = fem ? 0.094 : 0.082;
    const armR = fem ? 0.039 : 0.045;
    const legR = fem ? 0.061 : 0.066;
    const hip = [0, sway, 0.87 + bob];
    const out = [];
    const add = (c, pv, a, o, hb, ht, hz, yaw) =>
      out.push([c, pv, a, o, [hb[0], hb[1], ht[0], ht[1], hz], yaw || 0]);

    add(L.shorts, hip, 0, [0, 0, 0], [0.098, hipY + 0.030],
        [0.090, hipY + 0.020], 0.098, yawHip);
    add(L.jersey, hip, lean, [0, 0, 0.170], [0.076, shY * 0.74],
        [0.084, shY * 0.86], 0.086, yawTop);
    add(L.jersey, hip, lean, [0, 0, 0.352], [0.084, shY * 0.88],
        [0.092, shY * 0.99], 0.128, yawTop);
    add([242, 242, 238], hip, lean, [0.086, 0, 0.352], [0.010, shY * 0.46],
        [0.010, shY * 0.50], 0.060, yawTop);

    for (const side of [1, -1]) {
      add(L.skin, hip, lean, [0, side * shY, 0.462], [0.056, 0.044],
          [0.050, 0.038], 0.050, yawTop);
    }
    add(L.skin, hip, lean, [0, 0, 0.552], [0.042, 0.048], [0.040, 0.046],
        0.042, yawTop * 0.5);
    add(L.skin, hip, lean, [0.006, 0, 0.672 - bob * 0.55], [0.076, 0.074],
        [0.080, 0.078], 0.086, yawTop * 0.2);
    ...
    for (const [side, aArm, aFore] of [[1, al[0], al[1]], [-1, ar[0], ar[1]]]) {
      ...
      add(L.skin, S, aArm, [0, 0, -0.125], [armR - 0.007, armR - 0.004],
          [armR, armR + 0.004], 0.125, yawTop);
    ...
    for (const [side, th, sk, ft] of [[1, l[0], l[1], l[2]],
      ...
      add(L.skin, H, th, [0, 0, -0.190], [legR - 0.014, legR - 0.008],
          [legR + 0.004, legR + 0.010], 0.190, yawHip);
